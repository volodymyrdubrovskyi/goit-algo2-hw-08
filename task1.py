...
class SlidingWindowRateLimiter:
    ...
    def can_send_message(self, user_id: str) -> bool:
        ...
        return len(self.user_message_history.get(user_id, [])) < self.max_requests
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        current_time = time.time()
        self._cleanup_window(user_id, current_time)
        if len(self.user_message_history.get(user_id, [])) < self.max_requests:
            return 0
        return self.window_size - (current_time - self.user_message_history[user_id][0])
